        ...
        <div class="home-content">
            <div class="announcement-box">
                ...
                <ul>
                    ...
                    $society = $_SESSION["a_society"] ?? "0000";
                    $query = "SELECT * FROM events WHERE society_reg = '{$society}' AND event_start >= NOW() ORDER BY event_start LIMIT 5";
                    $result = mysqli_query($conn, $query);

                    if ($result && mysqli_num_rows($result) > 0) {
                        while ($row = mysqli_fetch_assoc($result)) {
                            echo "<li><strong class='event-title'>" . htmlspecialchars($row['event_title']) . "</strong>
                              <span class='event-date'>" . date('F j, Y, g:i A', strtotime($row['event_start'])) . "</span></li>";
                        }
                    } else {
                        echo "<li class='no-events'>No upcoming events.</li>";
                    }
                    ?>
                </ul>
            </div>
